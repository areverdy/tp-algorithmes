export const brief3lesmaths = () => {
    ...
    function additionboostee (a?: number, b?: number): number {
        let res;
        if(a === undefined){
            a = 0
        }
    ...
    function explose(nb: number): number[] {
        let res: number[] = []
        let nbToModulo = nb
    ...
    function additiongdnombre(a: number, b: number): number {
